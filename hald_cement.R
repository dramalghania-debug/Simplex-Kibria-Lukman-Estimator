...
# hald_cement.R
#
# Hald Cement application for the Simplex Kibria-Lukman Estimator (SKLE).
...
#
# The data are given in the script; no data file is required.
#
...
all_tables <- list()
cn_values <- list()
conv_values <- list()    # store convergence status
k_diag <- data.frame()    # store k diagnostics
...
for(lnk in links_to_test) {
  ...
  # k3: root of Equation (33), the largest k satisfying Theorem 2.
  # k4: square-root median estimator truncated at the regularity condition.
  lam_min2 <- 2 * min(vals)
  k3 <- max(1e-10, k_thm2)
  k4 <- max(1e-10, min(k2, lam_min2 * (1 - 1e-8)))
  k4_binds <- (k2 >= lam_min2)

  # Record the k values used.
  ...
  k_diag <- rbind(k_diag, data.frame(
    ...
    ratio_k1 = k1 / lam_min2, ratio_k2 = k2 / lam_min2,
...
for(lnk in links_to_test) {
  ...
  cat(paste0("   CONVERGED: ", cv$converged, "   (iterations: ", cv$iter, ")\n"))
...
# ---------------------------------------------------------------------------
# 8. INDEPENDENT CHECK OF THE FITTED COEFFICIENTS
#    Refits each model by direct numerical maximum likelihood and compares.
#    If the IRLS is correct, the two must agree to several decimal places.
# ---------------------------------------------------------------------------
cat(paste0("\n=========================================================================\n"))
...
for(lnk in links_to_test) {
  ...
  # two BFGS passes, restarting from the first solution, for a tight optimum
  op <- optim(rep(0, p_aug), nll, method = "BFGS",
